...
5. Write `parse_<endpoint>_results(json)` returning a `compact()` named list with snake_case keys derived from `paramName`.
6. Pass the function as the `result_fn` positional argument (third arg) in the `$new()` call. `token` must be passed as a named argument.