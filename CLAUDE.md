...
2. Ask the user to run the job and share `job$results` so we can see the `paramName`, `dataType`, and `value` columns.
3. Inspect `res$dataType` to identify which rows are `GPFeatureRecordSetLayer` (need `try_parse()`), which are scalar types like `GPBoolean`/`GPString` (use `RcppSimdJson::fparse()`), and which are `GPDataFile` (skip — will be `NULL`).
4. The raw results JSON is a **0-indexed array** — use numeric JSON Pointer paths (`"/0/value"`, `"/1/value"`, etc.) not name-based paths.
5. Write `parse_<endpoint>_results(json)` using `try_parse(json, "/N/value")` for feature layers and `RcppSimdJson::fparse(json, query = "/N/value")` for scalars. Return a `compact()` named list with snake_case keys.
