...
as_polyline_barriers.sfc <- function(x, ...) {
  ...
  if (length(x) > 500) {
    cli::cli_abort(
      "Only a maximum of 500 polyline barriers can be provided found {.val {length(x)}}"
    )
  }
...
as_polyline_barriers.sf <- function(x, ...) {
  ...
  if (n > 500) {
    cli::cli_abort(
      "Only a maximum of 500 polyline barriers can be provided found {.val {n}}"
    )
  }
  ...
  if (!geo_type %in% c("LINESTRING", "MULTILINESTRING")) {
    cli::cli_abort(
      "Polyline barriers must be a LINESTRING or a MULTILINESTRING not {.val {geo_type}}"
    )
  }
